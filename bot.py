...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL:
            try:
                for FORCE_SUB_CHANNELL in FORCE_SUB_CHANNEL:
                    link = (await self.get_chat(FORCE_SUB_CHANNELL)).invite_link
                    if not link:
                        await self.export_chat_invite_link(FORCE_SUB_CHANNELL)
                        link = (await self.get_chat(FORCE_SUB_CHANNELL)).invite_link
                ...
                self.LOGGER(__name__).warning(
                    f"Silakan periksa kembali var FORCE_SUB_CHANNEL dan Pastikan Bot anda Admin di Channel Tersebut dengan izin link invite Pengguna melalui link undangan, Chat ID F-Subs Channel Saat Ini: {FORCE_SUB_CHANNELL}"
                )
